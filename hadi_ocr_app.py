...
def normal_gravity_m_s2(latitude_deg: float, altitude_m: Optional[float] = None) -> float:
    """Equivalent local gravity from ASTM Table 1 MF.

    The rest of the app stores both g and MF. To make the W-row math match the
    ASTM table exactly, g is derived from:
        g = MF * 9.80665
    """
...
def load_sync_state() -> dict:
    if not SYNC_STATE_FILE.exists():
        return {"lag_ms": 0.0, "confidence": 0.0, "manual": False}
    try:
...
def save_sync_state(lag_ms: float, confidence: float, manual: bool = False) -> None:
    SYNC_STATE_FILE.write_text(
        json.dumps({"lag_ms": lag_ms, "confidence": confidence, "manual": manual}, indent=2),
        encoding="utf-8",
    )
...
class App(tk.Tk):
    ...
    def _build_capture_tab(self, root):
        ...
        ttk.Label(controls, text="Load Cell").grid(row=0, column=0, sticky="w", padx=(4, 2))
        ...
        self.load_cell_combo.grid(row=0, column=1, sticky="w", padx=(0, 10))
        ...
        mode = ttk.Combobox(controls, textvariable=self.mode_var, values=["Compression", "Tension"], state="readonly", width=14)
        mode.grid(row=0, column=3, sticky="w", padx=(0, 10))
        mode.bind("<<ComboboxSelected>>", lambda _e: self._set_mode())

        ttk.Label(controls, text="HADI Units").grid(row=0, column=4, sticky="w")
        units = ttk.Combobox(controls, textvariable=self.hadi_units_var, values=["LBF", "KGF", "N", "kN", "gF", "t", "mV/V"], state="readonly", width=8)
        units.grid(row=0, column=5, sticky="w", padx=(0, 10))
        units.bind("<<ComboboxSelected>>", lambda _e: self._set_hadi_display_options())

        ttk.Label(controls, text="Decimals").grid(row=0, column=6, sticky="w")
        decimals = ttk.Combobox(controls, textvariable=self.hadi_decimals_var, values=HADI_DECIMAL_OPTIONS, state="readonly", width=8)
        decimals.grid(row=0, column=7, sticky="w", padx=(0, 10))
        decimals.bind("<<ComboboxSelected>>", lambda _e: self._set_hadi_display_options())

        ttk.Button(controls, textvariable=self.hadi_button_var, command=self._toggle_hadi).grid(row=0, column=8, padx=6)
        ttk.Button(controls, textvariable=self.ocr_button_var, command=self._toggle_ocr).grid(row=0, column=9, padx=6)

        ttk.Label(controls, textvariable=self.status_var).grid(row=1, column=0, columnspan=5, sticky="w", padx=4, pady=(8, 0))
        ttk.Label(controls, textvariable=self.gps_status_var).grid(row=1, column=5, columnspan=5, sticky="e", padx=4, pady=(8, 0))
        ...
        self._big_label(left, "OCR Stream", self.ocr_value_var, 1)
        ...
        ttk.Button(
            left,
            text="SELECT / UNSELECT W",
            command=self._set_selected_manual_weight,
        ).grid(row=5, column=0, sticky="ew", padx=12, pady=(10, 2), ipady=8)

        ttk.Label(left, textvariable=self.weight_status_var, font=("Segoe UI", 9), wraplength=300, justify="left").grid(row=6, column=0, sticky="w", padx=12, pady=(2, 4))
        ...
        ttk.Button(btns, text="Set", command=self._apply_point_count).pack(side="left", padx=4)

        ttk.Label(btns, textvariable=self.count_var).pack(side="right")
        ttk.Label(btns, textvariable=self.target_var, font=("Segoe UI", 10, "bold")).pack(side="right", padx=14)
    ...
    def _hadi_live_label(self, parent, title_var: tk.StringVar, var: tk.StringVar, row: int):
        ...
        ttk.Label(header, textvariable=title_var, style="LiveTitle.TLabel").grid(row=0, column=0, sticky="w")
    ...
    def _auto_connect_on_launch(self):
        ...
        # so it is safe to run even before the phone starts streaming.
        if not self.ocr:
            self._start_ocr(show_error=False)

        # Try HADI automatically only if a COM port is available.
        if not self.hadi.is_connected():
            self._refresh_ports()
            if self.port_var.get():
                self._connect_hadi(show_error=False)
            else:
                self.status_var.set("OCR running. HADI waiting for COM port.")
    ...
    def _estimate_sync_lag(self):
        ...
        if len(hadi) < 12 or len(ocr) < 12:
            self.sync_status_var.set(f"Auto Sync: waiting for motion/data | lag {self.sync_lag_seconds*1000:+.0f} ms")
            return
        ...
        if (max(h_vals) - min(h_vals)) < 5 or (max(o_vals) - min(o_vals)) < 5:
            self.sync_status_var.set(f"Auto Sync: steady load, using saved lag {self.sync_lag_seconds*1000:+.0f} ms")
            return

        best_lag = None
        best_corr = -2.0

        # lag = OCR receive time minus HADI receive time for same event.
        # Positive means OCR path is later/slower than HADI.
        for lag_ms in range(-500, 501, 25):
            lag = lag_ms / 1000.0
        ...
        # Smooth the lag so it does not jump around off one noisy estimate.
        if best_corr >= 0.65:
            self.sync_lag_seconds = 0.80 * self.sync_lag_seconds + 0.20 * best_lag
            self.sync_confidence = best_corr
            save_sync_state(self.sync_lag_seconds * 1000.0, self.sync_confidence, manual=False)

        self.sync_status_var.set(
            f"Auto Sync lag {self.sync_lag_seconds*1000:+.0f} ms | confidence {self.sync_confidence:.2f}"
        )
    ...
    def _commit_ocr_edit(self, _event=None):
        ...
        if kind == "hadi":
            ...
            if was_weight:
                ...
                self.weight_status_var.set("HADI value entered. Select rows and click SELECT / UNSELECT W if needed.")
        else:
    ...
    def _set_selected_manual_weight(self):
        ...
        if toggled_on or toggled_off:
            self._mark_data_changed()
            self.weight_status_var.set(
                f"W updated: {toggled_on} on, {toggled_off} off (Run {run})"
            )
    ...
    def _set_selected_weight_reference(self):
        ...
        self._update_point_count_label()
        self.weight_status_var.set(
            f"Weight row set: {nominal_weight_lbf:g} lb ({gps_note})"
        )
